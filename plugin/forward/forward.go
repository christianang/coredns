...
func NewWithConfig(config ForwardConfig) (*Forward, error) {
	f := New()
	if config.From != "" {
		f.from = plugin.Host(config.From).Normalize()[0] // there can only be one here, won't work with non-octet reverse
	...
	for i := 0; i < len(config.Except); i++ {
		f.ignored = append(f.ignored, plugin.Host(config.Except[i]).Normalize()...)
	}
